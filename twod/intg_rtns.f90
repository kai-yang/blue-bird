...
!*********************************************************************  
subroutine sintg_2d(vo,v1,v2,lnp,one_ov_p)
...
!  one_ov_r : scalar potential contribution
  ...
  real(kind=dp),intent(out)::lnp,one_ov_p
  
  real(kind=dp)::vt1(2),vt2(2),rm,rp,vo1(2),vo2(2),ul(2),un1(2)
  real(kind=dp)::pp,pm,rlp,rlm,uu(2),p0,t1,betai

  lnp=0.0_dp; one_ov_p=0.d0
  vt1=v1; vt2=v2
  ...
  ! Compute length from P0 (projection of pl onto line of source edge)to vertices
  ! l+ (rlp) and l- (rlm) 
  uu=(/-ul(2),ul(1)/) ! uu is perpendicular to ul

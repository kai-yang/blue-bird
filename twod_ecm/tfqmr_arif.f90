...
subroutine dtfqmr_serial(ntotal,b,x,err,iter)
  use global_com
  use global_geom,only:nglunk
  use mat_vec_mult,only:matvec,r0_initial,precon
  implicit none
  integer,intent(in)::ntotal
  real(kind=dp),dimension(1:nglunk)::x,bb,b
  real(kind=dp)::err,rerr
  integer::iter,itmax,it,i
  real(kind=dp),dimension(1:nglunk)::w,yo,ayo,ye,aye,r,d,v
  real(kind=dp)::ta,we,cm
  real(kind=dp)::we_local,we_sum,rerr_local,rerr_sum,err_local,err_sum
  real(kind=dp)::ta_local,ta_sum,bmag_local,bmag_sum1,dumb_ali(6)
  real(kind=dp)::etha,rho,rho_local,amgis,amgis_local,ahpla,dum,dum_local,beta
  real(kind=dp)::bmag
